...
async def test_chat_message_success(client: AsyncClient):
    ...
    with patch("app.api.v1.chat.get_graph") as mock_get_graph:
        ...
        mock_graph.ainvoke = AsyncMock(return_value={
            ...
            "citations": [{"index": 1, "title": "Test Doc", "url": "http://test", "page": None}],
            "safety_tier": "safe"
        })
        ...
        response = await client.post(
            "/api/v1/chat/message",
            json={
                "message": "How do I start lifting?",
                "history": []
            },
            headers=headers
        )
...
async def test_chat_graph_failure(client: AsyncClient):
    ...
    with patch("app.api.v1.chat.get_graph") as mock_get_graph:
        from unittest.mock import AsyncMock
        mock_graph = MagicMock()
        mock_graph.ainvoke = AsyncMock(side_effect=Exception("Ollama connection refused"))
        mock_get_graph.return_value = mock_graph
