...
class TestRetrieve:
    ...
    def test_retrieve_returns_top_k(self, mock_vector, mock_embed):
        ...
        candidates = [(_make_chunk(i, f"fitness content for chunk {i}"), 0.9 - (0.01 * i)) for i in range(10)]
        mock_vector.side_effect = lambda query_embedding, db, top_k=TOP_K_FINAL: candidates[:top_k]
        ...
        assert results[0].vector_score > results[-1].vector_score  # Assuming sorted by score
    ...
    @patch("app.services.retrieval._embed_query", side_effect=ConnectionError("Ollama down"))
